...
export default function NewPassword() {
  ...
  // ---- Optional: update info banner from auth events ----
  useEffect(() => {
    const { data: sub } = supabase.auth.onAuthStateChange((event) => {
      if (event === "PASSWORD_RECOVERY") setInfo("You’ve verified your email. Create a new password below.");
      if (event === "SIGNED_IN") setInfo("You’re signed in. Create a password to finish setup.");
    });
    return () => sub.subscription.unsubscribe();
  }, []);

  // ---- Complexity checks (mirror Supabase's policy) ----
  const minLen = pw.length >= 8;
  const hasLower = /[a-z]/.test(pw);
  const hasUpper = /[A-Z]/.test(pw);
  const hasDigit = /\d/.test(pw);
  const hasSymbol = /[!@#$%^&*()_+\-=\[\]{};':"|<>?,.\/`~]/.test(pw); // includes /
  const meetsPolicy = minLen && hasLower && hasUpper && hasDigit && hasSymbol;

  const mismatch = pw && pw2 && pw !== pw2;

  const canSubmit = useMemo(() => {
    return !isSubmitting && meetsPolicy && !mismatch;
  }, [isSubmitting, meetsPolicy, mismatch]);
  ...
  async function handleSetPassword(e) {
    ...
    try {
      ...
      if (msg.includes("password should contain at least one character")) {
        setError("Password must include a-z, A-Z, 0-9, and a symbol.");
  ...
  // Small helper for the live checklist
  const Req = ({ ok, children }) => (
    <li className={`text-xs ${ok ? "text-emerald-300" : "text-white/50"}`}>
      <span className={`mr-2 inline-block h-4 w-4 rounded-full text-center text-[10px] leading-4 ${ok ? "bg-emerald-500 text-black" : "bg-white/10 text-white"}`}>
        {ok ? "✓" : "•"}
      </span>
      {children}
    </li>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      aria-invalid={!meetsPolicy && pw.length > 0}
                  ...
                  {/* Live checklist */}
                  <ul id="pw-reqs" className="mt-2 grid grid-cols-1 gap-1 sm:grid-cols-2">
                    <Req ok={minLen}>At least 8 characters</Req>
                    <Req ok={hasLower}>Contains a–z</Req>
                    <Req ok={hasUpper}>Contains A–Z</Req>
                    <Req ok={hasDigit}>Contains 0–9</Req>
                    <Req ok={hasSymbol}>Contains a symbol (!@#$…)</Req>
                  </ul>
                </div>
