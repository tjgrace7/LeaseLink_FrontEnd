...
// ⚠️ Lazy-load framer-motion to keep it off the critical path
let Motion = { motion: undefined, AnimatePresence: undefined };
...
/**
 * Lightweight SEO utility (moves DOM mutations to effects + idle time)
 * - Avoids blocking render during the commit phase
 * - Adds preload for LCP logo + OG image
 */
...
function useIsoEffect(effect, deps) {
  // Run after paint both on client and avoid doing anything during SSR
...
function Seo({ title, description, canonical, robots, og, twitter, jsonLd }) {
  useIsoEffect(() => {
    ...
    // JSON-LD — do this in idle time to avoid layout work on first paint
    const id = 'ld-home';
    const run = () => {
    ...
    // Hint: make above-the-fold images ultra-fast
    const preloadLogo = ensureTag('link[data-preload-logo]', () => {
      const l = document.createElement('link');
      l.setAttribute('rel','preload');
      l.setAttribute('as','image');
      l.setAttribute('data-preload-logo','');
      return l;
    });
    preloadLogo.setAttribute('href', logo);

    const preloadOg = ensureTag('link[data-preload-og]', () => {
      const l = document.createElement('link');
      l.setAttribute('rel','preload');
      l.setAttribute('as','image');
      l.setAttribute('imagesrcset','');
      l.setAttribute('data-preload-og','');
      return l;
    });
    preloadOg.setAttribute('href', OG_IMAGE);
  }, [title, description, canonical, robots, JSON.stringify(og), JSON.stringify(twitter), JSON.stringify(jsonLd)]);

  return null;
}

// Defer heavy libraries until after first paint
function useLazyMotion() {
  // No animations on initial load to minimize JS and CPU on mobile
  return {};


}

// Helper: content-visibility for below-the-fold sections to skip initial layout/paint
const cvStyle = { contentVisibility: 'auto', containIntrinsicSize: '1px 1200px' };

export default function HomePage() {
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const { motion, AnimatePresence } = useLazyMotion();
  ...
  // Motion shims (render static if not yet loaded)
  const M = motion || (({ children }) => children);
  const AP = AnimatePresence || (({ children }) => children);

  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      {/* SEO / Sharing (non-blocking) */}
      <Seo
      ...
      {/* Header */}
      <header className="sticky top-0 z-40 bg-neutral-950  border-b border-neutral-800" role="banner">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <img
              ...
              // Hint to prioritize this small but above-the-fold image
              fetchPriority="high"
              decoding="async"
            />
          ...
          <div className="hidden md:flex items-center gap-2">
            <button className="px-4 py-2 text-sm rounded-xl hover:bg-neutral-900" onClick={() => go("/login")}>
              Login
            </button>
        ...
        {/* Mobile sheet (renders without motion until loaded) */}
      ...
      {/* Main content wrapper to enable skip link */}
      <main id="main">
        {/* Hero — keep as the only above-the-fold heavy block */}
        <section className="relative overflow-hidden" id="story">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-24 grid lg:grid-cols-2 gap-8 lg:gap-10 items-center">
            ...
            >
              <div className="rounded-3xl shadow-md border border-neutral-800 bg-neutral-900 p-4 sm:p-6">
        ...
        {/* How it Works — below the fold: skip initial layout/paint */}
        <section id="how" className="bg-neutral-950 border-t border-neutral-800" style={cvStyle}>
        ...
        {/* Proof / Benefits */}
        <section id="proof" className="bg-neutral-950" style={cvStyle}>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-24">
        ...
        {/* CTA */}
        <section id="cta" className="relative bg-neutral-950 border-t border-neutral-800" style={cvStyle}>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-24">
      ...
      {/* Footer */}
      <footer className="bg-neutral-950 text-neutral-300" role="contentinfo" style={cvStyle}>
